...
import { iCreateTokenForCompany, iHasAuthenticationRecordCompany } from "../../../domain/usecases/authentications";
...
export class HasAuthenticationRecordCompanyData implements iHasAuthenticationRecordCompany {
    ...
    async exec(
        ...
        const foundedAuth : Auth = await this.authenticationRepository.getAuthByCredentials({
            email: input?.email,
            cnpj: input?.cnpj
        })
    ...
    private async validAuthForLogin(incomingAuth : Auth, foundedAuth : Auth){
        ...
        const {token} = await this.createTokenForCompany.exec({companyId : foundedAuth.associeteded_id})

        return {token};
    }

    private async validAuthExist(foundedAuth : Auth){
        if (foundedAuth) throw new UnauthorizedError('This CNPJ or Email has record, try change your passwoord.')
    }
}
