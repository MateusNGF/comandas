...
import { iCreateTokenForCompany, iHasAuthenticationRecordCompany } from "../../../../../src/domain/usecases/authentications";
...
describe('Has Authentication record for company', () => {
    ...
    it("Should return BadRequestError when has auth found.", async () => {
        ...
        const response = sut.exec(
            { ...fakeInputCredentials, password: "any_password" }
        );
    ...
    it("Should return valid token when has auth found and password is equals.", async () => {
        ...
        createTokenForCompany.exec.mockResolvedValue({ token : "this_is_valid_token"})

        const response = await sut.exec(
            { ...fakeInputCredentials, password: "any_password" }
        );

        expect(response).toEqual(expect.objectContaining({ token : "this_is_valid_token"}))
    })
