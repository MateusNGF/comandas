  ...
  private async inputProducts(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (productIncoming.name.length > settings.maxNameLength) {
        resultOperations[i] = makeError(
          `Very long product name at ${i}º, maximum ${settings.maxNameLength} characters.`
        );
        continue;
      }
      ...
      if (productIncoming.name.length < settings.minNameLength) {
        resultOperations[i] = makeError(
          `Very short product name at ${i}º, minimum ${settings.minNameLength} characters.`
        );
        continue;
      }
      ...
      if (productIncoming.description) {
        if (
          productIncoming.description.length > settings.maxDescriptionLength
        ) {
          resultOperations[i] = makeError(
            `Very long product description at ${i}º, maximum ${settings.maxDescriptionLength} characters.`
          );
          continue;
        }

        if (
          productIncoming.description.length < settings.minDescriptionLength
        ) {
          resultOperations[i] = makeError(
            `Very short product description at ${i}º, minimum ${settings.minDescriptionLength} characters.`
          );
          continue;
        }
      }
      ...
      if (productIncoming.sale_price < settings.minPriceSale) {
        resultOperations[i] = makeError(
          `Sale price for product at ${i}º cannot be less than ${settings.minPriceSale}.`
        );
        continue;
      }
      ...
      if (productIncoming.quantity < 1) {
        resultOperations[i] = makeError(
          `Quantity for product at ${i}º cannot be less than 1.`
        );
        continue;
      }

      if (
        productIncoming.buy_price &&
        productIncoming.buy_price < settings.minBuyPrice
      ) {
        resultOperations[i] = makeError(
          `Buy price for product at ${i}º cannot be less than ${settings.minBuyPrice}.`
        );
        continue;
      }

      const hasItemRecord = await this.inventoryRepository.findByName(
        productIncoming.name,
        company_id,
        options
      );
      if (hasItemRecord) {
        resultOperations[i] = makeError(
          `Item at ${i}º with name ${productIncoming.name} already exists.`
        );
        continue;
      }
      ...
      const runQueryInsert = await this.inventoryRepository.create(
        product,
        options
      );
